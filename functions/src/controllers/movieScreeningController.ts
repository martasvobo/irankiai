...
const createMovieScreening = onCall({ region: "europe-west1" }, async (request) => {
...
const updateMovieScreening = onCall({ region: "europe-west1" }, async (request) => {
  const { id, tickedCount, date, hall, movieId, cinemaId } = request.data as any;
  logger.info("Updating movie screening:", id, tickedCount, date, hall, movieId, cinemaId);
  try {
...
const deleteMovieScreening = onCall({ region: "europe-west1" }, async (request) => {
